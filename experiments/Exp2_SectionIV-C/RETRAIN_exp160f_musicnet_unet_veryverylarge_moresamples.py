...
from libdl.nn_models import simple_u_net_largekernels
...
# Specify and log model config #################################################
mp = model_params
model = simple_u_net_largekernels(n_chan_input=mp['n_chan_input'], n_chan_layers=mp['n_chan_layers'], \
    n_bins_in=mp['n_bins_in'], n_bins_out=mp['n_bins_out'], a_lrelu=mp['a_lrelu'], p_dropout=mp['p_dropout'], scalefac=mp['scalefac'])
model.to(device)
