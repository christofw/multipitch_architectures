...
from libdl.nn_models import u_net_blstm_varlayers
...
# Specify and log model config #################################################
mp = model_params
model = u_net_blstm_varlayers(n_chan_input=mp['n_chan_input'], n_chan_layers=mp['n_chan_layers'], \
    n_bins_in=mp['n_bins_in'], n_bins_out=mp['n_bins_out'], a_lrelu=mp['a_lrelu'], p_dropout=mp['p_dropout'], \
    scalefac=mp['scalefac'], embed_dim=mp['embed_dim'], hidden_size=mp['hidden_size'], \
    lstm_depth=mp['lstm_depth'], lstm_number=mp['lstm_number'])
model.to(device)
